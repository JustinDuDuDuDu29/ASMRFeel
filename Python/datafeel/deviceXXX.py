...
from pymodbus.pdu import device
import serial
...
class V63Registers():
    ...
    def __init__(self, dev):
        self.dev:modbusClient.ModbusBaseClient = dev

    @classmethod
    async def create(cls, port: str ):
        """Build the client and connect (async)."""
        dev = modbusClient.AsyncModbusSerialClient(
            port=port, framer=FramerType.RTU, retries=0,
    ...
    async def get_skin_temperature(self, id:int):
        """
        Get the skin temperature in Celsius.
        """
        # return self.dev.read_float(self.SKIN_TEMP, 3, 2, modbus.BYTEORDER_LITTLE_SWAP)
        res = await self.dev.read_holding_registers(address=self.SKIN_TEMP, count=2, device_id=id)
        print(self.dev.convert_from_registers(res.registers,word_order="little", data_type=self.dev.DATATYPE.FLOAT32))
        return res
    ...
    async def set_led_mode(self, mode: LedMode, id: int):
        """
        Set the LED mode.
        """
        print(f"Writing {id}")
        await self.dev.write_registers(address=self.LED_MODE, values=[int(mode)], device_id=id)
    ...
    async def set_global_led(self, red, green, blue, id):
        """
        ...
        await self.dev.write_registers(address=self.LED_MODE, values=[int(val)], device_id=id)
    ...
    async def get_global_led(self, id:int):
        """
        Get the global LED color.
        """
        # val = (blue << 16) | (red << 8) | green
        res = await self.dev.read_holding_registers(address=self.GLOBAL_MANUAL, count=2, device_id=id)
        # val = self.dev.read_long(self.GLOBAL_MANUAL, 3, False, modbus.BYTEORDER_LITTLE_SWAP, 2)
        
        print(f"val: {res}")
        val = self.dev.convert_from_registers(registers = res.registers, word_order="little", data_type= self.dev.DATATYPE.UINT32)
    ...
    async def set_individual_led(self, index, red, green, blue, id):
        """
        Set the individual LED color. Red, green and blue are between 0 and 255. Only valid when LED mode is INDIVIDUAL_MANUAL.
        """
        _val = (blue << 16) | (red << 8) | green
        val = self.dev.convert_to_registers(value=_val, data_type=self.dev.DATATYPE.INT32, word_order='little')

        
        print(f"wv {val}")

        await self.dev.write_registers(address=self.LED_INDIVIDUAL_MANUAL_0 + index * 2, values=val, device_id=id)
...
class Dot:
    ...
    async def set_led(self, red: int = None, green: int = None, blue: int = None, index: int = None):
        """
        ...
        """
        if index is None:
            await self.registers.set_led_mode(LedMode.GLOBAL_MANUAL,self.id)
            await self.registers.set_global_led(red, green, blue, self.id)
        else:
            await self.registers.set_led_mode(LedMode.INDIVIDUAL_MANUAL, self.id)
            await self.registers.set_individual_led(index, red, green, blue, self.id)

    async def set_led_breathe(self):
        """
        Set the LED to breathe.
        """
        await self.registers.set_led_mode(LedMode.BREATHE, self.id)

    async def set_led_off(self):
        """
        Set the LED to off.
        """
        await self.registers.set_led_mode(LedMode.OFF, self.id)
    ...
    def get_skin_temperature(self):
        ...
        Get the skin temperature in Celsius.
        """
        return self.registers.get_skin_temperature(self.id)
    ...
    def __init__(self, id, registers):
        self.id:int = id
        self.registers:V63Registers = registers
        self.device_name = None
        self.hardware_id =  None
        self.firmware_id = None
        self.serial_number = None

    async def _load_static_metadata(self):
        try:
            fix = _fix_string_endianness

            _device_name = await self.registers.dev.read_holding_registers(address=self.registers.DEVICE_NAME,count = 32, device_id = self.id)
            _hardware_id = await self.registers.dev.read_holding_registers(address=self.registers.HARDWARE_ID,count = 32, device_id = self.id)
            _firmware_id = await self.registers.dev.read_holding_registers(address=self.registers.FIRMWARE_ID,count = 32, device_id = self.id)
            _serial_number = await self.registers.dev.read_holding_registers(address=self.registers.SERIAL_NUMBER,count = 32, device_id = self.id)

            self.device_name = fix(self.registers.dev.convert_from_registers(_device_name.registers, data_type=modbusClient.ModbusBaseClient.DATATYPE.STRING))
            self.hardware_id = fix(self.registers.dev.convert_from_registers(_hardware_id.registers, data_type=modbusClient.ModbusBaseClient.DATATYPE.STRING))
            self.firmware_id = fix(self.registers.dev.convert_from_registers(_firmware_id.registers, data_type=modbusClient.ModbusBaseClient.DATATYPE.STRING))
            self.serial_number = fix(self.registers.dev.convert_from_registers(_serial_number.registers, data_type=modbusClient.ModbusBaseClient.DATATYPE.STRING))
    ...
    @classmethod
    async def create(cls, id: int, regs: V63Registers):
        """Build the client and connect (async)."""
        self = cls(id, regs)
        await self._load_static_metadata()
        return self
...
async def discover_devices(maxAddress) -> List[Dot]:
    """
    ...
    for port in serial.tools.list_ports.comports():
        if port.vid == 0x10c4 and port.pid == 0xea60:
            ...
            registers:V63Registers = await V63Registers.create(port.device)
            tasks = [Dot.create(i,registers) for i in range(1, maxAddress + 1)]
            results = await asyncio.gather(*tasks, return_exceptions=True)
            # devices = [res for res in results if not isinstance(res, Exception)]
            devices: List[Dot] = [r for r in results if isinstance(r, Dot)]
            # for x in range(4, maxAddress + 1):
            #     try:
            #         await registers.dev.read_device_information(device_id=x)
            #         print(f"adding {x}...")
            #         dot = await Dot.create(x, registers)
            #         devices.append(dot)
            #     except Exception as e:
            #         print(f"No device at address {x} {e}")
            break
    print(len(devices))
    return devices
